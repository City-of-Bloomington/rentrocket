...
urlpatterns = patterns(
    ...
    url(r'between/(?P<lat1>\d+\.\d+)/(?P<lng1>\d+\.\d+)/and/(?P<lat2>\d+\.\d+)/(?P<lng2>\d+\.\d+)/(?P<type>\w+)/(?P<limit>\d+)$', views.lookup, name='search'),
    ...
    url(r'in/(?P<city_tag>\w+)/matching/(?P<query>\w+)$', views.search),
    url(r'in/(?P<city_tag>\w+)/matching/$', views.search),
