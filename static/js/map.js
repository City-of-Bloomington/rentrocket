...
// RichMarker is used to display a custom marker during the map loading sequence
// if there is a simpler way to do that, this dependency can be removed. 
...
var RRMap = function(args) { 
  ...
  self.initialize = function () {
    ...
    //add a loading icon:
    //this will only show up until ajax call finishes
    //may need to comment out call to ShowMarkers to even see it
    var center = self.map.getCenter();

    var loading = new RichMarker({
      position: center,
      map: self.map,
      content: '<button class="btn btn-lg btn-warning"><div class="spinner"><div class="double-bounce1"></div> <div class="double-bounce2"></div></div>Loading...</button>',
    });

    self.markers.push(loading);    
    ...
    //http://stackoverflow.com/questions/18404948/preventing-google-maps-move-after-displaying-infowindow
    //could do a lot more to position the infowindow
    //in a way that is always on screen.
    //right now it's more annoying (and slow) that the window disappears
    self.infowindow.setOptions({disableAutoPan:true});
